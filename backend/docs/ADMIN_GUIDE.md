...
**Import et restauration reserves a `ROLE_ADMIN` (correctif securite ELEVEE, audit 04/08/2026).** `POST /api/v1/admin/backup/import` et `POST /api/v1/admin/backup/restore` (ainsi que leurs equivalents IHM) renvoient 403 pour un compte `ROLE_BACKUP` seul. Motif : importer un dump externe revient a injecter n'importe quel contenu dans la base applicative (`pg_restore --list` ne valide que la structure du format pg_dump, jamais son contenu), et une restauration ecrase l'integralite de la base — ces deux operations equivalaient donc de fait a un acces administrateur complet des donnees, malgre l'intention initiale d'un role limite pour compte de service.

**Configuration, purge et suppression egalement reservees a `ROLE_ADMIN` (correctif securite MOYENNE, second audit externe 04/08/2026).** `PUT /api/v1/admin/backup` (configuration), `POST .../purge` et `DELETE .../runs/{id}` (ainsi que leurs equivalents IHM, y compris la page de confirmation de suppression) renvoient desormais egalement 403 pour `ROLE_BACKUP` seul. Motif : un compte `ROLE_BACKUP` compromis pouvait desactiver la sauvegarde planifiee, alterer la retention/le cron, ou supprimer (en masse via purge, ou ligne par ligne) l'integralite de l'historique et des fichiers `.dump` sur disque — un sabotage silencieux de la capacite de reprise apres sinistre, sans jamais toucher a la base applicative elle-meme (donc distinct du correctif import/restore ci-dessus, mais tout aussi critique pour la resilience). `ROLE_BACKUP` conserve desormais uniquement : lecture de la configuration et de l'etat courant, declenchement manuel, consultation de l'historique (sauvegardes et restaurations) et telechargement — le strict necessaire pour un compte de service ou un operateur dedie qui execute et surveille des sauvegardes sans etre administrateur.
