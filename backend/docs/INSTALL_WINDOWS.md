...
dist\windows\subnetory-0.5.0-windows-x64.sha256.txt
dist\windows\subnetory-0.5.0-windows-x64.manifest.txt
```
...
```powershell
$zip = ".\dist\windows\subnetory-0.5.0-windows-x64.zip"
$expected = (Get-Content ".\dist\windows\subnetory-0.5.0-windows-x64.sha256.txt" | Select-Object -First 1).Split()[0]
