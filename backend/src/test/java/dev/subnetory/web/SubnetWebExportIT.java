...
/**
 * Tests d'intégration — Endpoints Web session export subnets (Sprint 2.9).
 *
 * <p>Vérifie que les endpoints {@code /network/subnets/export/csv} et
 * {@code /network/subnets/export/xlsx} sont :</p>
 * <ul>
...
class SubnetWebExportIT {
    ...
    @Test
    @DisplayName("GET /network/subnets/export/csv — anonyme → redirect /login")
    void exportCsv_anonymous_redirectsToLogin() throws Exception {
        mvc.perform(get("/network/subnets/export/csv"))
                .andExpect(status().is3xxRedirection())
                .andExpect(redirectedUrl("/login"));
    }

    @Test
    @DisplayName("GET /network/subnets/export/xlsx — anonyme → redirect /login")
    void exportXlsx_anonymous_redirectsToLogin() throws Exception {
        mvc.perform(get("/network/subnets/export/xlsx"))
                .andExpect(status().is3xxRedirection())
                .andExpect(redirectedUrl("/login"));
    }
    ...
    @DisplayName("GET /network/subnets/export/csv — 200 avec session, Content-Type text/csv")
    ...
    @Test
    @WithMockUser(roles = "NETWORK")
    @DisplayName("GET /network/subnets/export/csv — Content-Disposition attachment")
    ...
    @Test
    @WithMockUser(roles = "NETWORK")
    @DisplayName("GET /network/subnets/export/csv — header CSV présent")
    void exportCsv_withSession_hasHeader() throws Exception {
        MvcResult result = mvc.perform(get("/network/subnets/export/csv"))
                .andExpect(status().isOk())
                .andReturn();
    ...
    @DisplayName("GET /network/subnets/export/csv — accessible avec rôle ADMIN")
    ...
    @Test
    @WithMockUser(roles = "NETWORK")
    @DisplayName("GET /network/subnets/export/xlsx — 200 avec session, Content-Type xlsx")
    ...
    @Test
    @WithMockUser(roles = "NETWORK")
    @DisplayName("GET /network/subnets/export/xlsx — Content-Disposition attachment .xlsx")
    ...
    @Test
    @WithMockUser(roles = "NETWORK")
    @DisplayName("GET /network/subnets/export/xlsx — fichier xlsx parseable par Apache POI")
    void exportXlsx_withSession_isValidXlsx() throws Exception {
        MvcResult result = mvc.perform(get("/network/subnets/export/xlsx"))
                .andExpect(status().isOk())
                .andReturn();
    ...
    @DisplayName("GET /network/subnets/export/xlsx — accessible avec rôle IP")
    void exportXlsx_withIpRole_returns200() throws Exception {
        mvc.perform(get("/network/subnets/export/xlsx"))
                .andExpect(status().isOk());
    }
}
