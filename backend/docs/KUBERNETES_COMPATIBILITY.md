...
| Image applicative | `ghcr.io/subnetory/subnetory:v0.8.6` (défaut du chart) ou build local | Image publique publiée par le workflow de release, désormais le défaut de `image.repository`/`image.tag` dans `values.yaml` |
...
Depuis `v0.8.7` (troisième audit externe, constat M-03, 04/08/2026), l'image Subnetory officielle est signée "keyless" (Cosign/sigstore, identité GitHub OIDC) et accompagnée d'un SBOM et d'une attestation de provenance SLSA — voir `SECURITY.md`, section « Release integrity », pour les commandes de vérification. Ce n'est plus un élément non qualifié.
