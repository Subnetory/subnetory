...
| Image applicative | `ghcr.io/subnetory/subnetory:v0.8.5` (défaut du chart) ou build local | Image publique publiée par le workflow de release, désormais le défaut de `image.repository`/`image.tag` dans `values.yaml` |
